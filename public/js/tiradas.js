...
/* interpretación final por áreas: cada arcángel habla de su área en primera
  ...
  bloqueCombinacionGlobal(resultado) {
    ...
    if (inv === 0) {
      tipo = "normal";
      texto = `Tus cartas en conjunto cuentan una sola historia: ${protagonistas}. Todas brillan del derecho y empujan en la misma dirección, con ${resultado.cartas[0].nombre} marcando el paso desde ${tonoEje}. Yo, ${A}, te lo confirmo desde mi ${R}: lo que se combina a tu favor ya está en movimiento. Avanza con el corazón abierto.`;
    } else if (inv === total) {
      tipo = "sombra";
      texto = `Tus cartas en conjunto hablan en sombra: ${protagonistas}. Como todas se presentan invertidas, la jugada no es esperar sino voltear el enfoque: ${resultado.cartas[0].nombre} invertida te pide soltar ${tonoEje}. Yo, ${A}, te lo digo desde mi ${R}: nada de esto es un no, es un aviso a tiempo para dar la vuelta.`;
    } else {
      tipo = "espejada";
      const luz = resultado.cartas.filter(c => !c.invertido)[0];
      const sombra = resultado.cartas.find(c => c.invertido);
      texto = `Tus cartas en conjunto se miran de frente: ${protagonistas}. Mientras ${luz.emoji} ${luz.nombre} te muestra lo que ya avanza, ${sombra.emoji} ${sombra.nombre} invertida te devuelve lo que aún pide atención; la lección pasa por ${tonoEje}. Yo, ${A}, te digo desde mi ${R} que este espejo es un regalo: escucha las dos caras, equilibra y la balanza se inclinará a tu favor.`;
    }
    return {
      icono: "🔗",
      area: "combinacion",
